...
(arduinoGenerator as any).finish = function (code: string): string {
  ...
  // Inject AI hat block calls into the loop
  let processedCode = code;
  if (this.definitions_['ai_serial_reader']) {
    const aiHatCalls = Object.keys(this.definitions_)
      .filter((key) => key.startsWith('ai_hat_'))
      .map((key) => `  ${key.replace('ai_hat_', '')}();`)
      .join('\n');

    if (processedCode.includes('void loop() {')) {
      const loopInjection = `void loop() {\n  updateAISerial();\n${aiHatCalls}`;
      processedCode = processedCode.replace('void loop() {', loopInjection);
    } else {
      processedCode += `\nvoid loop() {\n  updateAISerial();\n${aiHatCalls}\n}\n`;
    }

    if (processedCode.includes('void setup() {')) {
  ...
  // Board-aware header: use the correct include for the selected board
  const board = useEditorStore.getState().board;
  let include = '#include <Arduino.h>';
  if (board === 'esp8266:esp8266:nodemcuv2') {
    include = '#include <ESP8266WiFi.h>';
  }
  let header = `// Generated by Tinkergyan Block Editor\n${include}\n`;
  if (processedCode.includes('LED_BUILTIN') || (varDecls && varDecls.includes('LED_BUILTIN'))) {
    header += `\n#ifndef LED_BUILTIN\n#define LED_BUILTIN 2\n#endif\n`;
  }
  return [header, varDecls ? varDecls + '\n' : '', processedCode].filter(Boolean).join('\n');
};
