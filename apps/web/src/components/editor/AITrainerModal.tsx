...
import { X, Camera, Trash2, Plus, Brain, Loader2, Video, VideoOff } from 'lucide-react';
...
export function AITrainerModal({ isOpen, onClose }: AITrainerModalProps) {
  ...
  const syncTrainingState = useCallback(() => {
    ...
    updateTrainingState(allLabels, counts);
  ...
  const handleClose = useCallback(() => {
    ...
    // Keep webcam running if vision blocks need it
    onClose();
  }, [handleCaptureStop, isTesting, setPredicting, onClose]);

  if (!isOpen) return null;

  const totalSamples = Object.values(exampleCounts).reduce((a, b) => a + b, 0);
  const trainedClassCount = Object.values(exampleCounts).filter((c) => c > 0).length;
  const canTest = trainedClassCount >= 2;

  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="relative w-[95vw] max-w-[900px] max-h-[90vh] bg-[#1a1a2e] border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/50 text-xs">
                Train your own image classifier — no coding needed
              </p>
            </div>
          </div>
          <button
                        ...
                        <div className="flex-1 h-3 rounded-full bg-white/10 overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${conf}%`,
                              backgroundColor:
                                label === currentPrediction ? '#FF6F61' : 'rgba(255,255,255,0.2)',
                            }}
                          />
                        </div>
